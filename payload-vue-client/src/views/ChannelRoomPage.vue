<template>
  <IonPage>
    <IonHeader :translucent="true" class="gradient-custom">
      <IonToolbar>
    ...
    <IonContent :fullscreen="true">
      ...
      <section class="gradient-custom m-2">
        <div class="container py-2">
          <div class="title">
            <h3 class="text-center" style="margin-left: 16px; font-weight: bold; font-size: 30px">Channel : {{ channelName }}</h3>
          </div>
          <div class="row">
            <ul class="list-unstyled">
              <li v-for="(message, index) in messages" :key="index" class="text-white d-flex justify-content-center">
                <div class="card mask-custom" style="width: 50%">
                  <div class="card-header d-flex justify-content-between p-3 m-1" style="border-bottom: 1px solid rgba(255, 255, 255, 0.3); border-radius: 50px">
                    <img :src="'https://picsum.photos/200/200?random=${randomIndex(people)' + ((index % 10) + 1)" alt="avatar" class="rounded-circle d-flex align-self-start me-3 shadow-1-strong" width="60" />
  ...
  setup() {
    ...
    const fetchMessages = async () => {
      try {
        ...
        messages.value = data.docs.filter((message) => message.channels.id === id.value);
